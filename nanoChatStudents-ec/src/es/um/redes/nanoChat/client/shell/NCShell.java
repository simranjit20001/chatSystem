...
public class NCShell {
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NCCommands.COM_ENTER:
			...
			break;
		//creatrRoom requiere el parámetro <room>
		case NCCommands.COM_CHANGE_ROOM_NAME:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: change_room_name <name>");
				return false;
			}
			break;
		...
		case NCCommands.COM_SEND:
			...
			break;
		//makeAdmin requiere el parámetro <nickname>
		case NCCommands.COM_MAKE_ADMIN:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: makeAdmin <nickname>");
				return false;
			}
			break;
		//removeAdmin requiere el parámetro <nickname>
		case NCCommands.COM_REMOVE_ADMIN:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: removeAdmin <nickname>");
				return false;
			}
			break;
		case NCCommands.COM_KICK:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: kick <nickname>");
				return false;
			}
			break;
		case NCCommands.COM_SEND_PRIVATE:
			if (args.length == 0 || args.length !=  2) {
				System.out.println("Correct use: send_private <nickname> <message>");
				return false;
			} 
			break;
		case NCCommands.COM_CREATE_ROOM:
